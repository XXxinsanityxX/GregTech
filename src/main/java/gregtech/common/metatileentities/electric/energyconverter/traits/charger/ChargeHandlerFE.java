...
import gregtech.common.metatileentities.electric.energyconverter.energy.ElectricItemFE;
import gregtech.common.metatileentities.electric.energyconverter.energy.EnergyStorageGTEU;
...
public class ChargeHandlerFE extends ChargeHandler {
	...
	protected IEnergyStorage getItemEnergyStorage(final ItemStack s, final boolean wrapEUBattery) {
		...
		if (wrapEUBattery) {
			...
			return (i != null && (!ConfigHolder.energyConverter.PermitOnlyExactVoltage || i.getTier() == this.energyConverter.getTier())) ? new EnergyStorageGTEU(this.energyConverter, i) : null;
		}
		return null;
	}
}
